...
class tx_jfpowermailfrontend_edit extends tslib_pibase
{
	var $extKey = 'jfpowermailfrontend';
	var $prefixId = 'tx_jfpowermailfrontend_pi1';
	var $scriptRelPath = 'pi1/class.tx_jfpowermailfrontend_edit.php';
	var $mode = 'edit';
	var $dbInsert = 1;
		...
		if ($this->piVars['edit'] > 0) { // only if GET param show was set
			...
			if (empty($this->piVars['change']['uid'])) { // no values to save, so show form
				...
				if (!$this->div->allowed($this->piVars['edit'], $this->conf)) { // current user is allowed to make changes
					...
					$this->vars = $this->div->allowedUID($this->vars, $this->conf['edit.']['fields']);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/jfpowermailfrontend/pi1/class.tx_jfpowermailfrontend_edit.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/jfpowermailfrontend/pi1/class.tx_jfpowermailfrontend_edit.php']);
}
