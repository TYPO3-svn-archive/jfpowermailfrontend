...
require_once(t3lib_extMgm::extPath('jfpowermailfrontend') . 'pi1/class.tx_jfpowermailfrontend_list.php'); // load list class
require_once(t3lib_extMgm::extPath('jfpowermailfrontend') . 'pi1/class.tx_jfpowermailfrontend_single.php'); // load single class
require_once(t3lib_extMgm::extPath('jfpowermailfrontend') . 'pi1/class.tx_jfpowermailfrontend_search.php'); // load search class
require_once(t3lib_extMgm::extPath('jfpowermailfrontend') . 'pi1/class.tx_jfpowermailfrontend_edit.php'); // load edit class
...
/**
 * Plugin 'jfpowermailfrontend' for the 'jfpowermailfrontend' extension.
 *
 * @author	Alex Kellner <alexander.kellner@in2code.de>
 * @package	TYPO3
 * @subpackage	tx_jfpowermailfrontend_pi1
 */
class tx_jfpowermailfrontend_pi1 extends tslib_pibase
{
				...
				for ($i=0; $i < count($mode); $i++) { // One loop for every selected mode
					switch ($mode[$i]) { // mode
						case 'list': // if list mode is selected
							$this->list = t3lib_div::makeInstance('tx_jfpowermailfrontend_list'); // Create new instance for list class
						...
						case 'single': // if single mode is selected
							$this->single = t3lib_div::makeInstance('tx_jfpowermailfrontend_single'); // Create new instance for single class
							$this->content .= $this->single->main($this->conf, $this->piVars, $this->cObj); // use single class
							break;
						case 'search': // if category mode is selected
							$this->search = t3lib_div::makeInstance('tx_jfpowermailfrontend_search'); // Create new instance for search class
							$this->content .= $this->search->main($this->conf, $this->piVars, $this->cObj); // use search class
							break;
						case 'edit': // if edit mode is selected
							$this->edit = t3lib_div::makeInstance('tx_jfpowermailfrontend_edit'); // Create new instance for edit class
	...
	private function check()
	{
		...
		if (intval($this->cObj->data['pages']) === 0) { // if startingpoint is not set
			$error = '<b>' . $this->pi_getLL('pi1_error', 'jfpowermailfrontend error: No startingpoint was set in backend!') . '</b><br />'; // errormessage
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/jfpowermailfrontend/pi1/class.tx_jfpowermailfrontend_pi1.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/jfpowermailfrontend/pi1/class.tx_jfpowermailfrontend_pi1.php']);
}
